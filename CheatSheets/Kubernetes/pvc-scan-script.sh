...
#CHANGE KUBECONFIG FILE IF YOU DON'T HAVE IT
function getNodes() {
  kubectl --kubeconfig ./kubeconfig get --raw $NODESAPI | jq -r '.items[].metadata.name'
}
...
for node in $(getNodes); do
  kubectl --kubeconfig ./kubeconfig get --raw $NODESAPI/$node/proxy/stats/summary
done | getPVCs | format