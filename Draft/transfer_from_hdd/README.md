Here's a script that copies your terminal directories with progress tracking, dry-run capability, and comprehensive logging:
...
1. **Terminal Counting**: Scans all directories first to get total count
2. **Progress Display**: Shows `STATUS: Processed 4500/13000 terminals` in real-time
3. **Dry-run Mode**: Set `DRYRUN=false` to actually copy files
4. **Logging**: Comprehensive logging with timestamps
5. **Versioning**: Includes version tracking
6. **Lock File**: Prevents multiple instances from running
7. **Error Handling**: Tracks successful and failed copies
8. **Efficient Copying**: Uses `rsync` for reliable file copying

**Usage:**

1. Save the script: `nano copy_terminals.sh`
2. Make executable: `chmod +x copy_terminals.sh`
3. Test first with dry-run: `./copy_terminals.sh` (DRYRUN=true)
4. Check logs in `./logs/` directory
5. When ready, set `DRYRUN=false` and run again

**Important Notes:**

- The script uses `rsync -a` which preserves permissions, timestamps, and copies recursively
- Progress is shown in real-time on terminal and logged
- Every 100 terminals, a checkpoint is logged
- Failed copies are tracked and reported
- The lock file prevents accidental parallel runs

Would you like me to adjust the progress update frequency or add any other features?