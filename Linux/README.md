...
- **systemd-networkd**: Linux systems that utilize the systemd initialization method normally use this daemon to detect network interfaces and automatically create entries for them in the network configuration files.
- No single standard configuration file exists that all distributions use.
  - **Debian-based**: `/etc/network/interfaces` file
  - **Red Hat–based**: `/etc/sysconfig/network-scripts` directory
  - **OpenSUSE**: `/etc/sysconfig/network` file

You need to define a DNS server so that the system can use DNS host names.
...
- For legacy SysVinit systems, that’s handled in the `/etc/resolv.conf` configuration file:
  - `domain [mydomain.com](http://mydomain.com)` → Defines the domain name assigned to the network. By default, the system will append this domain name to any host names you specify.
  - `search [mytest.com](http://mytest.com)` → Defines any additional domains used to search for host names.
  - `nameserver 192.168.1.1` → Specifies the DNS server assigned to your network.
...
#### Legacy tools (utilities from the net-tools package)
...
- `$ ip (iproute2 Package)` → Show / manipulate routing, network devices, interfaces, and tunnels
- `/etc/sysctl.conf` → Tune networking parameters for a network interface. The Linux system uses this when interacting with the network interface, such as to disable responding to ICMP messages by setting the `icmp_echo_ignore_broadcasts` value to 1, or if your system has multiple network interface cards, to disable packet forwarding by setting the `ip_forward` value to 0.
